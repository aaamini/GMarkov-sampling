...
inline_legend = function(x = .8, y = .2) {
...
append_avg = function(res, col) {
  avg_res = res %>% 
    ...
    group_by(lambda, method) %>% summarise("{{col}}" := mean({{col}})) %>% 
    add_column(rep = "avg", alpha = 1, size = 1)
  
  res %>% select(lambda, method, rep, {{col}}) %>% 
...
plot_paths_and_avg = function(res, col, alpha_range = c(0.25,1)) {
  res %>% 
    append_avg({{col}}) %>% 
    ggplot(aes(x = lambda, y = {{col}}, color = method, alpha = alpha)) +
